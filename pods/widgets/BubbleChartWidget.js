	...
	AjaxSolr.BubbleChartWidget = AjaxSolr.AbstractFacetWidget.extend({
		afterRequest: function () 
		{
			...
			if (this.manager.response.facet_counts.facet_fields[this.field] === undefined ||
				jQuery.isEmptyObject(this.manager.response.facet_counts.facet_fields[this.field])
			) {
				...
				for (var facet in this.manager.response.facet_counts.facet_fields[this.field]) {
				  ...
				  objectedItems2.push({ name: facet, children : [{name: getLanguageText(facet), size: count}] });
				}
				  ...
				  node.append("text")
					  ...
					  .on("click", function(d) { //clickhandler function
						  var self = env, meth = env.multivalue ? 'add' : 'set';			
							self[meth].call(self, getLanguageCode(d.className));
							self.doRequest();
					   }); 
